...
class GAIN(nn.Module):
    def __init__(self, grad_layer, num_classes, device):
    ...
    def _to_ohe(self, labels):
        ...
        ohe = torch.nn.functional.one_hot(labels.long(), self.num_classes).sum(dim=0).unsqueeze(0)
        ohe = torch.autograd.Variable(ohe)
    ...
    def forward(self, images, labels):
        ...
        with torch.enable_grad():
            ...
            if not is_train:
                ...
                labels_ohe = self._to_ohe(pred).to(self.device)
            else:
                #print(labels)
                labels_ohe = self._to_ohe(labels).to(self.device)

            gradient = logits * labels_ohe
            grad_logits = (logits * labels_ohe).sum()  # BS x num_classes
            grad_logits.backward(gradient=torch.ones_like(grad_logits), retain_graph=True)
            self.model.zero_grad()
